...
void Block::Update(float dt)
{
	...
	transform.position += m_forwardV * m_speed * dt;
	...
	transform.position.x = cg::Clamp(transform.position.x, 0.0f, (float)cg::g_renderer.getWidth());
	transform.position.y = cg::Clamp(transform.position.y, 0.0f, (float)cg::g_renderer.getHeight());
	if (transform.position.y >= (float)cg::g_renderer.getHeight() && !m_placed) {
		Place();
	}
}
...
void Block::OnCollision(Actor* other)
{
	...
	if (transform.position.y <= 50 && m_placed && !m_destroyed) {
		dynamic_cast<AstroidFighter*>(m_game)->TetrisMaxHeightReached();
		m_destroyed = true;
	}
	if (other->tag == "Laser" && transform.position.y <= 30 && !m_placed && !m_destroyed) {
		m_destroyed = true;
...
bool Block::Destroy()
{
	...
	cg::Transform transform{ transform.position, 0, 1 };
	auto emitter = std::make_unique<cg::Emitter>(transform, data);
	emitter->lifespan = cg::randomf(0.05f, 0.15f);
	//m_scene->Add(std::move(emitter));
